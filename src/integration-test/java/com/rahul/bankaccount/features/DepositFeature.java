...
public class DepositFeature extends CucumberRoot {
    ...
    @Given("^a client tries to deposit Rs ([0-9]*) to accountId ([A-Z0-9]*)$")
    public void given(BigDecimal amount, String accountId) {
        this.amount = amount;
        this.accountId = accountId;
    }

    @When("^the client deposits the amount by making a POST call to (.*)$")
    public void step1(String url) {
        url = enrich(url);
        bankTransaction = doPost(url, amount);
    }

    @Then("^for the deposit transaction the client receives response code of (\\d+)$")
    ...
    @And("^for the deposit transaction the client receives a banking transaction " +
...
//            String successful = jsonObject.get("successful").getAsString();
